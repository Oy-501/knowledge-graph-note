...
export function extractTerms(text) {
  const terms = []; const lowerText = text.toLowerCase()
  for (const term of COMMON_TERMS) {
    const lowerTerm = term.toLowerCase(); let pos = 0
    while ((pos = lowerText.indexOf(lowerTerm, pos)) !== -1) {
      ...
      if (/\W/.test(before) && /\W/.test(after)) terms.push({ term, position: pos, length: term.length })
      pos += lowerTerm.length
    }
  }
  const seen = new Set()
  return terms.filter(t => { const k = `${t.term}:${t.position}`; if (seen.has(k)) return false; seen.add(k); return true }).sort((a, b) => a.position - b.position)
}

export function matchGraphNodes(terms, graphNodes) {
  if (!terms.length || !graphNodes.length) return []
  const matched = []; const matchedIds = new Set()
  for (const { term } of terms) {
    ...
    for (const node of graphNodes) {
      ...
      if (node.title && node.title.toLowerCase() === lowerTerm) { matched.push({ node, matchType: 'exact_title', term }); matchedIds.add(node.id); continue }
      if (node.title && node.title.toLowerCase().includes(lowerTerm)) { matched.push({ node, matchType: 'title_contains', term }); matchedIds.add(node.id); continue }
      if (nodeKeywords.includes(lowerTerm) || nodeEntities.includes(lowerTerm)) { matched.push({ node, matchType: 'keyword_match', term }); matchedIds.add(node.id); continue }
      if (nodeText.includes(lowerTerm) && lowerTerm.length > 3) { matched.push({ node, matchType: 'description_contains', term }); matchedIds.add(node.id) }
    }
  }
  return matched
}
...
export function extractRelations(text) {
  const relations = []; const terms = extractTerms(text)
  ...
  for (const [keyword, relInfo] of Object.entries(RELATION_KEYWORDS)) {
    ...
    if (beforeTerms.length > 0 && afterTerms.length > 0) {
      ...
      relations.push({ source, target, relation: relInfo, evidence: text.slice(evidenceStart, evidenceEnd).trim() })
    }
  }
  return relations
}
...
export function parseMarkdownSummary(content) {
  ...
  const lines = content.split('\n'); const headings = []; let paragraphCount = 0
  for (const line of lines) {
    const trimmed = line.trim()
    if (/^#{1,4}\s+/.test(trimmed)) headings.push(trimmed.replace(/^#+\s+/, ''))
    else if (trimmed.length > 10) paragraphCount++
  }
  return { summary: headings.length > 0 ? headings.join(' · ') : content.slice(0, 100), headings, paragraphCount }
}

export function extractTags(content) {
  if (!content) return []
  const tags = []; const tagRegex = /#[\u4e00-\u9fa5\w]+/g; let match
  while ((match = tagRegex.exec(content)) !== null) {
    const tag = match[0].replace(/^#/, '').trim()
    if (tag.length > 1 && tag.length < 30 && !tags.includes(tag)) tags.push(tag)
  }
  return tags
}
