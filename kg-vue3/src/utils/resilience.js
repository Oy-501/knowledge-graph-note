...
export class TaskQueue {
  ...
  get progress() {
    return this._totalTasks > 0 ? Math.round((this._completedTasks / this._totalTasks) * 100) : 0
  }
  ...
  async process() {
    ...
    const results = await Promise.allSettled(
      batch.map(item => this._execute(item))
    )
...
/**
 * 分批渲染，每批之间让出主线程
 * @param {Array} items - 待渲染数据
 * @param {Function} renderFn - 渲染函数 (batch, batchIndex) => void
...
export class VersionedStore {
  constructor(maxHistory = 50, snapshotInterval = 5) {
    ...
    this._takeSnapshot = null
    this._restoreSnapshot = null
    this._onSave = null
  }

  /** 设置快照/恢复/保存回调 */
  configure({ takeSnapshot, restoreSnapshot, onSave }) {
    this._takeSnapshot = takeSnapshot
    this._restoreSnapshot = restoreSnapshot
    this._onSave = onSave
  }

  mutate(operation) {
    if (this._takeSnapshot) {
    ...
    this.operationCount++
    if (this.operationCount % this.snapshotInterval === 0 && this._onSave) {
      this._onSave()
    }
  ...
  undo() {
    ...
    if (previous && this._restoreSnapshot) {
...
export class GracefulDegradation {
  ...
  async executeWithFallback(operations) {
    // operations: [{ fn, level }] 按优先级排列
    for (let i = 0; i < operations.length; i++) {
      try {
        return await operations[i].fn()
      } catch (e) {
        if (i < operations.length - 1) {
          this.level = operations[i + 1].level || 'reduced'
          if (this._onLevelChange) this._onLevelChange(this.level, e.message)
        }
      }
    }
    return null
  ...
  getStatusMessage() {
    const messages = {
      'full': '系统运行正常',
      'reduced': '系统进入降级模式，部分功能不可用',
      'minimal': '系统进入基础模式，仅支持核心功能'
    }
...
export class Transaction {
  ...
  async execute() {
    ...
    try {
      ...
      if (this.snapshot && this.store.restoreSnapshot) {
...
export class IntegrityGuard {
  ...
  checkAndRepair() {
    ...
    // 检查1：所有连线的 source/target 必须存在
    const validLinks = []
    for (const link of links) {
      const s = typeof link.source === 'object' ? link.source.id : link.source
      const t = typeof link.target === 'object' ? link.target.id : link.target
      if (!nodeIds.has(s)) {
        issues.push({ type: 'dangling_link_source', linkId: link.id, missing: s })
        continue
      }
      if (!nodeIds.has(t)) {
        issues.push({ type: 'dangling_link_target', linkId: link.id, missing: t })
        continue
      }
    ...
    if (removedLinks > 0) {
      this.store.links = validLinks
      issues.push({ type: 'repaired_dangling_links', count: removedLinks })
    }

    // 检查2：isolateBlackList 中的 ID 必须存在
    for (const node of nodes) {
      if (!node.isolateBlackList?.length) continue
      const valid = node.isolateBlackList.filter(id => nodeIds.has(id))
      if (valid.length !== node.isolateBlackList.length) {
        const removed = node.isolateBlackList.length - valid.length
        node.isolateBlackList = valid
        issues.push({ type: 'repaired_blacklist', nodeId: node.id, removed })
      }
    }
    ...
    if (this.store.groups) {
      ...
      for (const g of this.store.groups) {
        const expected = counts[g.id] || 0
        if (g.count !== expected) {
          g.count = expected
          issues.push({ type: 'repaired_group_count', groupId: g.id, expected, was: g.count })
        }
      }
    }
...
export class HealthMonitor {
  ...
  startMonitoring(intervalMs = 5000) {
    ...
    this._intervalId = setInterval(() => {
      ...
      if (this._onUpdate) this._onUpdate(this.metrics, this.warnings)
    }, intervalMs)
  }
  ...
  collectMetrics() {
    this.metrics.memoryUsageMB = performance.memory?.usedJSHeapSize
      ? Math.round(performance.memory.usedJSHeapSize / 1024 / 1024)
      : 0
    this.metrics.errorCount = _errorLog.length
  }

  updateStoreMetrics(nodes, links, pendingTasks) {
    this.metrics.nodeCount = nodes?.length || 0
    this.metrics.linkCount = links?.length || 0
    this.metrics.pendingTasks = pendingTasks || 0
  }

  checkThresholds() {
    this.warnings = []
    if (this.metrics.nodeCount > 500) {
      this.warnings.push({ level: 'info', msg: '知识点超过500个，建议使用分组功能提升性能' })
    }
    if (this.metrics.nodeCount > 1000) {
      this.warnings.push({ level: 'warn', msg: '知识点超过1000个，图谱渲染可能变慢' })
    }
    if (this.metrics.pendingTasks > 10) {
      this.warnings.push({ level: 'warn', msg: '有大量任务排队中，请耐心等待' })
    }
    if (this.metrics.memoryUsageMB > 500) {
      this.warnings.push({ level: 'warn', msg: `内存占用较高（${this.metrics.memoryUsageMB}MB），建议刷新页面释放缓存` })
    }
    if (this.metrics.errorCount > 20) {
      this.warnings.push({ level: 'error', msg: '错误数量较多，建议重启应用' })
    }
  }
...
/**
 * 韧性删除文件：事务化 + 完整清理 + 自愈
 * @param {Object} store - graphStore 实例
...
export async function resilientDeleteFile(store, fileId) {
  ...
  tx.add(async () => {
    ...
    for (const node of store.nodes) {
      if (node.isolateBlackList?.length) {
        node.isolateBlackList = node.isolateBlackList.filter(id => !nodeIds.includes(id))
      }
    }
  ...
  try {
    ...
    logError('deleteFile', fileId, e)
    return { success: false, issues: [], error: e.message }
  }
}