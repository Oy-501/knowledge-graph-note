<template>
  <div class="fws-panel">
    ...
    <div v-if="!folder.supported" class="fws-unsupported">
      ...
      <p class="fws-us-sub">
        本地文件夹笔记需要 File System Access API，请使用最新版
        <b>Chrome / Edge</b>，并在 <b>https 或 localhost</b> 环境下打开本应用。
      </p>
    ...
    <div v-else-if="folder.workspaceCount === 0" class="fws-empty">
      ...
      <p class="fws-empty-sub">
        选择任意含 Markdown 笔记的文件夹，系统会：
        读取本地 <code>.md</code> 为权威数据源、自动写入
        <code>.kg_meta/kg.json</code> 元数据、并在右侧编辑器直接编辑。
      </p>
      <button type="button" class="fws-btn fws-btn-primary" :disabled="folder.loading" @click="onPick">
        {{ folder.loading ? '加载中…' : '📂 打开本地文件夹' }}
      </button>
    </div>
    ...
    <template v-else>
      ...
      <div class="fws-header">
        <div class="fws-header-row">
          <select
            v-if="folder.workspaceCount > 1"
            class="fws-ws-select"
            :value="folder.currentWorkspaceId"
            title="切换工作区"
            @change="onSwitchWs"
          >
            <option v-for="ws in folder.workspaces" :key="ws.id" :value="ws.id">
              {{ ws.rootName }}
            </option>
          </select>
          <span v-else class="fws-ws-name" :title="folder.currentWorkspace?.rootName">
            🗂 {{ folder.currentWorkspace?.rootName }}
          </span>
        ...
        <div v-if="folder.currentOpenNote" class="fws-open-status" :class="'fws-os-' + saveStateKey">
          <span class="fws-os-dot"></span>
          <span class="fws-os-text">{{ saveStateText }}</span>
          <span v-if="folder.isDirty" class="fws-os-hint">（约 1 秒后自动保存）</span>
          <span
            v-if="syncBadgeText"
            class="fws-sync-badge"
            :class="'fws-sync-' + syncBadgeKey"
        ...
        <div class="fws-actions">
          <button type="button" class="fws-btn fws-btn-small fws-btn-primary" :disabled="folder.loading" @click="onNewNote">
            📄 新建笔记
          </button>
          <span
            v-if="scanCountText"
            class="fws-scan-report"
            :title="folder.lastScanReport?.missing.map(m => m.relPath).join('\n') || ''"
          >
            {{ scanCountText }}
          </span>
        </div>
      ...
      <div v-else-if="treeRows.length === 0" class="fws-tree-empty">
        目录内暂无 Markdown 笔记，点击「新建笔记」开始
      </div>
      <div v-else class="fws-tree" @scroll.passive>
        ...
        <div
          ...
          :key="row.relPath"
          class="fws-row"
          :class="[row.kind, { 'fws-row-active': isRowActive(row) }]"
          :style="{ paddingLeft: 6 + row.depth * 14 + 'px' }"
          @click="row.kind === 'dir' ? onToggleDir(row) : emitOpen(row.relPath)"
        >
          <template v-if="row.kind === 'dir'">
            <span class="fws-chevron" @click.stop="onToggleDir(row)">
              {{ isDirOpen(row) ? '▾' : '▸' }}
            </span>
            <span class="fws-row-icon">📁</span>
            <span class="fws-row-name">{{ row.name }}</span>
            <span class="fws-dir-count">{{ row.fileCount }}</span>
            <span class="fws-row-ops" @click.stop>
              <button type="button" class="fws-op" title="重命名目录" @click="onRenameDir(row)">✎</button>
            </span>
          </template>
          <!-- 笔记行 -->
          <template v-else>
            ...
            <span class="fws-row-name" :title="row.relPath">
              {{ row.name }}
              <span v-if="rowDirty(row)" class="fws-dirty-dot" title="有未保存修改"></span>
            </span>
            <span class="fws-row-ops" @click.stop>
...
const MAX_ROWS = 300
const expanded = reactive(new Set()) // 展开的目录 relPath

// ==================== 树行计算 ====================

/** 将目录树拍平成带缩进的行（目录优先显示），并计算每个目录内笔记数（computed 内只算行数据，不写响应式状态） */
const treeRows = computed(() => {
  const rows = []

  const dirFileCount = node => {
    let c = 0
    const walk = n => {
      for (const ch of n.children || []) {
        if (ch.kind === 'dir') walk(ch)
        else c += 1
      }
    }
    walk(node)
  ...
  const walk = (node, depth) => {
    ...
    for (const child of node?.children || []) {
      ...
      if (child.kind === 'dir') {
        rows.push({
          type: 'dir', kind: 'dir', relPath: child.relPath, name: child.name,
          depth, fileCount: dirFileCount(child)
        })
        if (expanded.has(child.relPath)) walk(child, depth + 1)
      } else {
        rows.push({ type: 'file', kind: 'file', relPath: child.relPath, name: child.name, depth })
      }
    }
  }
  walk(folder.tree, 0)
...
/** 截断判定：与 treeRows 同一 walk 规则的独立只读计算，取代渲染期写 ref */
const treeTruncated = computed(() => {
  ...
  const countWalk = node => {
    for (const child of node?.children || []) {
      ...
      count += 1
      if (child.kind === 'dir' && expanded.has(child.relPath)) {
        if (countWalk(child)) return true
      }
    }
    return false
  }
...
function onToggleDir(row) {
  if (expanded.has(row.relPath)) expanded.delete(row.relPath)
  else expanded.add(row.relPath)
}

/** 当前打开文件且未保存的标记（dirty） */
const openRelPath = computed(() => folder.currentOpenNote?.relPath || null)
function isRowActive(row) {
  return row.kind === 'file' && row.relPath === openRelPath.value
}
function rowDirty(row) {
  return row.relPath === openRelPath.value && folder.saveState === 'dirty'
}
...
const syncBadgeKey = computed(() => {
  const n = folder.currentOpenNote
  if (!n) return ''
...
const syncBadgeText = computed(() => {
  const n = folder.currentOpenNote
  if (!n) return ''
  if (n.syncError) return '⚠ 知识库同步失败'
  return n.backendFileId ? '🗄 已入库' : '☁ 首次保存入库'
})
const syncBadgeTitle = computed(() => {
  const n = folder.currentOpenNote
  if (!n) return ''
  if (n.syncError) return '后端不可达，保存仍成功；后端恢复后再次保存将自动重试。' + (n.syncError || '')
  if (n.backendFileId) return '内容已双写知识库；删除此笔记时知识节点保留'
  return '离线可用；首次保存将把本文档登记入知识库并抽取知识节点'
})

const scanCountText = computed(() => {
  const rep = folder.lastScanReport
  if (!rep) return ''
  const n = (rep.added?.length || 0)
  if (n > 0) return `发现 ${n} 个未登记文件`
  return ''
})
...
async function onPick() {
  ...
  if (r?.ok) {
    expanded.clear()
    ElMessage.success(r.reused ? `已重新打开工作区「${r.workspace.rootName}」` : `已打开工作区「${r.workspace.rootName}」`)
...
async function onRescan() {
  ...
  if (r?.ok) {
    expanded.clear()
    if ((r.added?.length || 0) === 0 && (r.missing?.length || 0) === 0) {
      ElMessage.success('目录已是最新')
    }
  } else {
    ElMessage.warning(r?.msg || '重扫失败')
  }
}
...
async function onNewNote() {
  let name = ''
  try {
    const { value } = await ElMessageBox.prompt('笔记标题（作为文件名）', '新建本地笔记', {
      inputValue: '',
      inputPlaceholder: '未命名笔记',
  ...
  if (r?.ok) {
    ElMessage.success('已创建本地笔记')
    emitOpen(r.relPath)
  } else {
    ElMessage.warning(r?.msg || '创建失败')
  }
}

async function onRenameFile(row) {
  try {
    const { value } = await ElMessageBox.prompt('输入新文件名（含扩展名）', '重命名笔记', {
      inputValue: row.name,
    ...
    if (r?.ok) {
      ...
      if (r.relPath !== row.relPath) emitOpen(r.relPath) // 保持打开
    } else {
      ElMessage.warning(r?.msg || '重命名失败')
    }
  } catch (e) { /* 取消 */ }
}
...
async function onDeleteFile(row) {
  ElMessageBox.confirm(
    `删除笔记「${row.name}」？\n将删除本地文件；若已入库，知识节点会保留（彻底删除请在后续模块操作）。`,
    '删除笔记',
    {
...
async function onRenameDir(row) {
  try {
    const { value } = await ElMessageBox.prompt('输入新目录名', '重命名目录', {
      inputValue: row.name,
      confirmButtonText: '重命名',
      cancelButtonText: '取消'
    })
    ...
    if (e?.message) ElMessage.warning('重命名失败：' + e.message)
  }
}

async function onCloseNote() {
  // 最新内容已由编辑器实时写入 currentOpenNote（KnowledgeWorkbench.onContentInput → setDraft），
  // 关闭前先同步落盘，防抖草稿不丢失；残留防抖 timer 由 KnowledgeWorkbench 的
  // currentOpenNote watch 在置空后统一 cancel
  const r = await folder.saveNote()
  if (r?.ok === false && r.msg && r.msg !== '没有打开的笔记') {
    ElMessage.warning(r.msg)
  }
...
/** 由组件内动作（新建/重命名后）通知父级刷新编辑器 */
function emitOpen(relPath) {
  setTimeout(() => emit('select-folder-note', relPath), 0)
}

onMounted(async () => {
  // 自动恢复上次使用的工作区（句柄持久化在 IndexedDB fs_roots）
  if (folder.supported && folder.workspaceCount === 0 && !folder.currentWorkspaceId) {
    await folder.init()
  }
  // 目录全折叠是默认态；若有已打开文件，将其所在路径展开以便定位
...
.fws-icon-btn:hover:not(:disabled) {
  background: var(--bg-hover);
  color: var(--text-primary);
}
.fws-icon-btn:active:not(:disabled) {
...
.fws-sync-err {
  ...
  background: color-mix(in srgb, var(--danger, #e84c4c) 10%, transparent);
}
