  ...
  <div class="kb-view">
    ...
    <header class="kb-head">
      <div class="kb-head-main">
        <h2>知识库</h2>
        <p>
          知识库是系统的知识权威源：文档先<strong>锚定</strong>到知识库知识点，
          再由知识库<strong>概念空间</strong>决定文件之间、知识点之间的关联 ——
          不是拿文本向量比相似度，没被知识库覆盖的内容不产生关联。
        </p>
      </div>
      <div class="kb-head-actions">
        <span class="kb-threshold">
          关联阈值
          <el-slider v-model="kbStore.rebuildThreshold" :min="0.05" :max="0.5" :step="0.05"
                     style="width:120px" size="small" />
        </span>
        <el-button type="primary" :loading="kbStore.rebuilding" @click="onRebuild">
          重建知识关联
        </el-button>
      </div>
    </header>
    ...
    <el-tabs v-model="activeTab" class="kb-tabs">
      ...
      <el-tab-pane label="概览" name="overview">
        <div class="kb-stat-grid">
          <div class="kb-stat" v-for="card in statCards" :key="card.label">
            ...
            <div class="ks-sub">{{ card.sub }}</div>
          </div>
        </div>

        <div class="kb-two-col">
          <section class="kb-card">
            <h3>领域分布 <span class="kb-count">{{ kbStore.domains.length }}</span></h3>
            <div class="kb-bars">
              <div v-for="d in kbStore.domains" :key="d.domain" class="kb-bar-row">
                <span class="kb-bar-name" :title="d.domain">{{ d.domain }}</span>
                <div class="kb-bar-track">
                  <div class="kb-bar-fill" :style="{ width: barWidth(d.count) }"></div>
                </div>
                <span class="kb-bar-val">{{ d.count }}</span>
              </div>
              <div v-if="!kbStore.domains.length" class="kb-empty">暂无知识点</div>
            </div>
          </section>
          ...
          <section class="kb-card">
            ...
            <h3 style="margin-top:14px">来源构成</h3>
            <div class="kb-sources">
              <el-tag v-for="(count, key) in (kbStore.stats?.sources || {})" :key="key"
                      size="small" effect="plain" class="kb-src-tag">
                {{ key }} · {{ count }}
              </el-tag>
            </div>
          </section>
        ...
        <section class="kb-card">
          <h3>
            知识库本体图
            <span class="kb-count">{{ kbStore.graph.nodes.length }} 点 / {{ kbStore.graph.links.length }} 边</span>
            <span class="kb-inline-tools">
              <el-select v-model="graphDomain" size="small" style="width:150px" clearable
                         placeholder="全部领域" @change="loadGraph">
                <el-option v-for="d in kbStore.domains" :key="d.domain"
                           :label="d.domain" :value="d.domain" />
              </el-select>
              <el-radio-group v-model="colorMode" size="small">
                <el-radio-button label="domain">按领域</el-radio-button>
                <el-radio-button label="level">按层级</el-radio-button>
              </el-radio-group>
            </span>
          </h3>
          <KbOntologyCanvas :nodes="kbStore.graph.nodes" :links="kbStore.graph.links"
                            :color-mode="colorMode" :height="380" />
          <p class="kb-hint">
            虚线边＝由条目自身的「相关术语 / 桥接句」自动推导；实线边＝用户上传或手工维护。
          </p>
        </section>
      </el-tab-pane>
      ...
      <el-tab-pane label="上传理解" name="ingest">
        <div class="kb-two-col">
          <section class="kb-card">
            <h3>上传知识库文件</h3>
            <div class="kb-dropzone" :class="{ dragover: dragging }"
                 @click="pickFile"
                 @dragover.prevent="dragging = true"
                 @dragleave.prevent="dragging = false"
                 @drop.prevent="onDrop">
              <div class="dz-icon">📚</div>
              <div class="dz-text">拖拽知识库文件到此处，或点击选择</div>
              <div class="dz-sub">支持 .md（条目式/表格）· .csv/.tsv（含表头）· .json · .txt</div>
              <input ref="fileRef" type="file" hidden
                     accept=".md,.markdown,.txt,.csv,.tsv,.json"
                     @change="onFileChange" />
            </div>

            <div class="kb-formats">
              <div class="kb-fmt-title">识别规则</div>
              <ul>
                <li><b>markdown</b>：<code>## 实体 (别名)</code> + <code>别名:</code>/<code>关系:</code>/<code>bridge_sentence:</code>/<code>domain:</code></li>
                <li><b>csv/tsv</b>：表头含 实体 / 别名 / 领域 / 层级 / 定义 / 关系</li>
                <li><b>json</b>：<code>[{"entity": "...", "definition": "..."}]</code></li>
                <li><b>txt</b>：每行 <code>实体：定义</code></li>
              </ul>
            </div>

            <el-checkbox v-model="overwriteDefs" size="small">
              定义冲突时用上传内容覆盖知识库既有定义
            </el-checkbox>

            <div v-if="pendingFile" class="kb-pending">
              <span class="kp-pending-text">《{{ pendingFile.name }}》已生成预览报告（尚未入库）</span>
              <el-button size="small" type="primary" :loading="kbStore.importing"
                         @click="confirmPending">导入知识库</el-button>
          ...
          <section class="kb-card">
            <h3>或粘贴文本理解</h3>
            <el-input v-model="pasteText" type="textarea" :rows="7"
                      placeholder="把知识库内容粘贴到这里…" />
            <div class="kb-paste-actions">
              <el-button :loading="kbStore.previewing" @click="onPreview">预览理解</el-button>
              <el-button type="primary" :loading="kbStore.importing"
                         :disabled="!pasteText.trim()" @click="onImportPaste">导入知识库</el-button>
            </div>
          </section>
        </div>

        <section class="kb-card" v-if="kbStore.report">
          <h3>
            理解报告
            <span class="kb-count">{{ kbStore.report.format }}</span>
            <span class="kb-inline-tools">
              <el-tag v-if="kbStore.report.dry_run" type="warning" size="small" effect="dark">预览（未入库）</el-tag>
            </span>
          </h3>
          <p class="kb-summary">{{ kbStore.report.summary }}</p>
          <div class="kb-report-grid">
            <div class="kb-report-block">
              <div class="krb-title ok">新增知识点 {{ kbStore.report.new_entries?.length || 0 }}</div>
              <div class="krb-body">
                <span v-for="e in kbStore.report.new_entries" :key="e.entity" class="kb-chip ok"
                      :title="e.definition">{{ e.entity }}</span>
                <span v-if="!kbStore.report.new_entries?.length" class="kb-empty">无</span>
              </div>
            </div>
            <div class="kb-report-block">
              <div class="krb-title info">同义合并 {{ kbStore.report.merged_entries?.length || 0 }}</div>
              <div class="krb-body">
                <span v-for="m in kbStore.report.merged_entries" :key="m.entity" class="kb-chip info"
                      :title="`新增别名：${(m.added_aliases || []).join('、') || '—'}｜新增关系：${(m.added_related || []).join('、') || '—'}`">
                  {{ m.entity }}
                </span>
                <span v-if="!kbStore.report.merged_entries?.length" class="kb-empty">无</span>
              </div>
            </div>
            <div class="kb-report-block">
              <div class="krb-title warn">定义冲突 {{ kbStore.report.conflicts?.length || 0 }}</div>
              <div class="krb-body">
                <div v-for="c in kbStore.report.conflicts" :key="c.entity" class="kb-conflict">
                  <div class="kc-entity">{{ c.entity }} <span class="kc-res">{{ c.resolution }}</span></div>
                  <div class="kc-line"><b>知识库</b>：{{ c.existing }}</div>
                  <div class="kc-line"><b>上传</b>：{{ c.incoming }}</div>
                </div>
                <span v-if="!kbStore.report.conflicts?.length" class="kb-empty">无</span>
              </div>
            </div>
            <div class="kb-report-block">
              <div class="krb-title">未采纳 {{ kbStore.report.rejected?.length || 0 }}
                <span class="kb-relations-note">新增关系 {{ (kbStore.report.relations_added?.length || 0) + (kbStore.report.relations_derived || 0) }}</span>
              </div>
              <div class="krb-body">
                <span v-for="(r, i) in kbStore.report.rejected" :key="i" class="kb-chip muted"
                      :title="r.reason">{{ r.entity }}（{{ r.reason }}）</span>
                <span v-if="!kbStore.report.rejected?.length" class="kb-empty">无</span>
              </div>
            </div>
          </div>
          <div class="kb-report-tail">
            知识库现有 {{ kbStore.report.stats?.kb_total || 0 }} 个知识点 ·
            {{ kbStore.report.stats?.relations_total || 0 }} 条关系
          </div>
        </section>

        <section class="kb-card">
          <h3>上传历史 <span class="kb-count">{{ kbStore.imports.length }}</span></h3>
          <el-table :data="kbStore.imports" size="small" @row-click="onOpenImport" style="cursor:pointer">
            <el-table-column prop="name" label="文件" min-width="180" show-overflow-tooltip />
            <el-table-column prop="format" label="格式" width="90" />
            <el-table-column prop="entry_total" label="识别" width="70" />
            <el-table-column prop="new_count" label="新增" width="70" />
            <el-table-column prop="merged_count" label="合并" width="70" />
            <el-table-column prop="conflict_count" label="冲突" width="70" />
            <el-table-column prop="relation_count" label="关系" width="70" />
      ...
      <el-tab-pane label="知识点" name="entries">
        <section class="kb-card">
          <h3>
            知识点管理
            <span class="kb-count">{{ kbStore.entryTotal }}</span>
            <span class="kb-inline-tools">
              <el-input v-model="kbStore.entryFilter.keyword" size="small" style="width:170px"
                        placeholder="搜索知识点" clearable @keyup.enter="searchEntries" />
              <el-select v-model="kbStore.entryFilter.domain" size="small" style="width:140px"
                         placeholder="全部领域" clearable>
                <el-option v-for="d in kbStore.domains" :key="d.domain"
                           :label="d.domain" :value="d.domain" />
              </el-select>
              <el-select v-model="kbStore.entryFilter.level" size="small" style="width:110px">
                <el-option label="全部层级" :value="0" />
                <el-option v-for="l in [1,2,3,4]" :key="l" :label="`L${l}`" :value="l" />
              </el-select>
              <el-button size="small" @click="searchEntries">筛选</el-button>
              <el-button size="small" type="primary" @click="openEditor(null)">新增知识点</el-button>
            </span>
          </h3>

          <el-table :data="kbStore.entries" size="small" v-loading="kbStore.loadingEntries">
            <el-table-column prop="entity" label="知识点" min-width="150" show-overflow-tooltip>
              <template #default="{ row }">
                <span class="kb-entity" @click="showNeighbors(row)">{{ row.entity }}</span>
              </template>
            </el-table-column>
            <el-table-column label="别名" min-width="140">
              <template #default="{ row }">
                <span v-if="row.aliases?.length">{{ row.aliases.join('、') }}</span>
                <span v-else class="kb-empty">—</span>
              </template>
            </el-table-column>
            <el-table-column prop="domain" label="领域" width="120" show-overflow-tooltip />
          ...
          <div class="kb-pager">
            <el-button size="small" :disabled="entryOffset === 0" @click="pageTo(-1)">上一页</el-button>
            <span class="kb-page-info">{{ entryOffset + 1 }} - {{ Math.min(entryOffset + 50, kbStore.entryTotal) }} / {{ kbStore.entryTotal }}</span>
            <el-button size="small" :disabled="entryOffset + 50 >= kbStore.entryTotal" @click="pageTo(1)">下一页</el-button>
          </div>
        </section>
      </el-tab-pane>
      ...
      <el-tab-pane label="知识关联" name="linking">
        <section class="kb-card">
          <h3>知识锚定测试 <span class="kb-count">文本 → 命中的知识点</span></h3>
          <el-input v-model="matchText" type="textarea" :rows="4"
                    placeholder="粘贴一段文本，看看知识库认出了哪些知识点（这就是「锚定」）" />
          <div class="kb-paste-actions">
            <el-button :loading="kbStore.matching" @click="onMatch">知识锚定</el-button>
            <span v-if="kbStore.matchResult" class="kb-match-meta">
              命中 {{ kbStore.matchResult.matched }} 个知识点 ·
              {{ kbStore.matchResult.hits }} 次出现 ·
              知识库覆盖率 {{ (kbStore.matchResult.coverage * 100).toFixed(1) }}%
            </span>
          </div>
          <div class="kb-match-list" v-if="kbStore.matchResult?.concepts?.length">
            <div v-for="c in kbStore.matchResult.concepts" :key="c.entity" class="kb-match-item">
              <span class="km-name">{{ c.entity }}</span>
              <span class="km-meta">{{ c.domain }} · L{{ c.level }} · 出现 {{ c.count }} 次</span>
              <span v-if="c.via_alias" class="km-alias">经别名「{{ c.matched_text }}」命中</span>
              <div class="km-def">{{ c.definition }}</div>
            </div>
          </div>
          <p v-else-if="kbStore.matchResult" class="kb-hint">
            该文本没有命中任何知识库知识点 —— 按知识边界规则，它不会参与任何关联。
          </p>
        </section>
        ...
        <section class="kb-card">
          <h3>
            文件知识画像
            <span class="kb-count">{{ kbStore.fileProfiles.length }}</span>
            <span class="kb-inline-tools">
              <span class="kb-hint">平均知识库覆盖率 {{ (kbStore.avgCoverage * 100).toFixed(1) }}%</span>
            </span>
          </h3>
          <div class="kb-profile-grid">
            <div v-for="f in kbStore.fileProfiles" :key="f.file_id" class="kb-profile"
                 :class="{ uncovered: !f.concept_count }">
              <div class="kp-head">
                <span class="kp-name" :title="f.name">{{ f.name }}</span>
                <el-tag size="small" :type="f.concept_count ? 'success' : 'info'" effect="plain">
                  {{ f.concept_count }} 个知识点
                </el-tag>
              </div>
              <el-progress :percentage="Math.round((f.coverage || 0) * 100)"
                           :stroke-width="6" :show-text="false"
                           :color="f.concept_count ? 'var(--mint)' : 'var(--text-muted)'" />
              <div class="kp-coverage">
                知识库覆盖率 {{ ((f.coverage || 0) * 100).toFixed(1) }}%
                · {{ f.node_count }} 个文档节点
              </div>
              <div class="kp-concepts">
                <span v-for="c in f.top_concepts" :key="c.entity" class="kb-chip info"
                      :title="`权重 ${c.weight}｜出现 ${c.count} 次`">{{ c.entity }}</span>
                <span v-if="!f.top_concepts?.length" class="kb-empty">未被知识库覆盖</span>
              </div>
            </div>
          </div>
        </section>
        ...
        <section class="kb-card">
          <h3>
            文件间知识关联
            <span class="kb-count">{{ kbStore.fileLinks.length }} 条 · 阈值 {{ kbStore.rebuildThreshold }}</span>
          </h3>
          <p class="kb-hint">
            综合分 = 共享知识点 50% + 知识库关系桥接 35% + 领域/层级一致性 15%。
            字面相似但知识库不认的文件不会出现在这里。
          </p>
          <div v-for="l in sortedFileLinks" :key="l.id" class="kb-link-item">
            <div class="kl-head">
              <span class="kl-files">
                <b>{{ l.source_name }}</b> × <b>{{ l.target_name }}</b>
              </span>
              <span class="kl-score">{{ (l.kb_similarity * 100).toFixed(1) }}%</span>
            </div>
            <div class="kl-scores">
              共享 {{ (l.direct_score * 100).toFixed(0) }}% ·
              桥接 {{ (l.bridge_score * 100).toFixed(0) }}% ·
              领域层级 {{ (l.domain_score * 100).toFixed(0) }}%
            </div>
            <div class="kl-block" v-if="l.shared_concepts?.length">
              <span class="kl-label">共同知识点</span>
              <span v-for="c in l.shared_concepts" :key="c" class="kb-chip ok">{{ c }}</span>
            </div>
            <div class="kl-block" v-if="l.bridges?.length">
              <span class="kl-label">知识库通路</span>
              <div v-for="(b, i) in l.bridges.slice(0, 4)" :key="i" class="kl-bridge">
                <span class="klb-path">{{ b.from }} <i>{{ b.relation_type }}</i> {{ b.to }}</span>
                <span class="klb-hops">{{ b.hops }} 跳</span>
                <span class="klb-evidence">{{ b.evidence }}</span>
              </div>
            </div>
          </div>
          <div v-if="!kbStore.fileLinks.length" class="kb-empty">
            暂无文件间知识关联（上传更多文档，或下调关联阈值后重建）
          </div>
          <div v-if="kbStore.uncoveredFiles.length" class="kb-uncovered">
            ⚠ 以下文件未被知识库覆盖，按知识边界规则不参与关联：
            {{ kbStore.uncoveredFiles.map(f => f.name).join('、') }}
          </div>
        </section>
      </el-tab-pane>
    </el-tabs>
    ...
    <!-- 知识点编辑弹窗 -->
    <el-dialog v-model="editorVisible" :title="editorForm.id ? '编辑知识点' : '新增知识点'" width="560px">
      <el-form :model="editorForm" label-width="72px" size="small">
        <el-form-item label="知识点">
          <el-input v-model="editorForm.entity" placeholder="如：JavaScript" />
        </el-form-item>
        <el-form-item label="别名">
          <el-input v-model="editorAliasText" placeholder="多个别名用 、或 , 分隔" />
        </el-form-item>
        <el-form-item label="领域">
          <el-input v-model="editorForm.domain" placeholder="如：frontend" />
        </el-form-item>
        <el-form-item label="层级">
          <el-radio-group v-model="editorForm.level">
            <el-radio-button v-for="l in [1,2,3,4]" :key="l" :label="l">L{{ l }}</el-radio-button>
          </el-radio-group>
        </el-form-item>
        <el-form-item label="定义">
          <el-input v-model="editorForm.definition" type="textarea" :rows="4" />
        </el-form-item>
        <el-form-item label="相关术语">
          <el-input v-model="editorRelatedText" placeholder="多个术语用 、或 , 分隔" />
        </el-form-item>
      </el-form>
      <template #footer>
    ...
    <!-- 知识点邻域弹窗 -->
    <el-dialog v-model="neighborVisible" :title="`「${neighborEntity}」在知识库中的邻域`" width="560px">
      <div v-for="n in neighbors" :key="n.entity" class="kb-neighbor">
        <div class="kn-head">
          <span class="kn-name">{{ n.entity }}</span>
          <el-tag size="small" effect="plain">{{ n.relation_type }}</el-tag>
          <span class="kn-weight">强度 {{ n.weight }}</span>
        </div>
        <div class="kn-evidence">{{ n.evidence || n.definition }}</div>
      </div>
      <div v-if="!neighbors.length" class="kb-empty">该知识点在知识库中没有关系边</div>
    </el-dialog>
  </div>
</template>
...
const levelRows = computed(() => {
  ...
  return [1, 2, 3, 4].map(k => ({
    key: k,
    label: LEVEL_META[k].label,
    color: LEVEL_META[k].color,
    value: levels[String(k)] || 0
  }))
})

const sortedFileLinks = computed(() =>
  [...kbStore.fileLinks].sort((a, b) => b.kb_similarity - a.kb_similarity)
)

const maxDomainCount = computed(() =>
  Math.max(1, ...kbStore.domains.map(d => d.count))
)

function barWidth(count) {
  return `${Math.max(2, (count / maxDomainCount.value) * 100)}%`
}
...
function loadGraph() {
  kbStore.loadGraph({ domain: graphDomain.value, limit: 150 })
}

function searchEntries() {
  entryOffset.value = 0
  kbStore.loadEntries({ offset: 0 })
}

function pageTo(direction) {
  entryOffset.value = Math.max(0, entryOffset.value + direction * 50)
  kbStore.loadEntries({ offset: entryOffset.value })
}
...
async function handleFile(file) {
  ...
  if (report) {
    ...
    ElMessage.info('已生成理解报告，确认无误后点「导入知识库」')
  }
}
...
async function onOpenImport(row) {
  await kbStore.loadImportDetail(row.id)
  activeTab.value = 'ingest'
  ElMessage.success(`已载入《${row.name}》的理解报告`)
}
...
function openEditor(row) {
  if (row) {
    editorForm.value = {
      id: row.id, entity: row.entity, domain: row.domain || 'general',
      level: row.level || 3, definition: row.definition || ''
    }
...
async function saveEntry() {
  if (!editorForm.value.entity.trim()) {
    ElMessage.warning('请填写知识点名称')
    return
  }
  const payload = {
    entity: editorForm.value.entity.trim(),
    domain: editorForm.value.domain || 'general',
    level: editorForm.value.level,
    definition: editorForm.value.definition,
    aliases: splitList(editorAliasText.value),
    related_terms: splitList(editorRelatedText.value),
    credibility: 6
  }
...
async function onDeleteEntry(row) {
  try {
    await ElMessageBox.confirm(
      `删除知识点「${row.entity}」会同时删除它的知识库关系边，已锚定该知识点的文档节点不会被删除。确认删除？`,
      '删除知识点', { type: 'warning' }
    )
  } catch (e) { return }
...
async function onMatch() {
  if (!matchText.value.trim()) {
    ElMessage.warning('请先粘贴一段文本')
    return
  }
...
.kb-dropzone {
  ...
  background: var(--bg-tertiary); transition: all var(--dur-fast) var(--ease-out);
}
...
.kb-link-item {
  border-left: 3px solid var(--accent); background: var(--bg-tertiary);
