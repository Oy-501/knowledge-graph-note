...
LEGACY_SEVERITY_MAP = {'error': 'critical', 'high': 'critical', 'warning': 'major', 'medium': 'major', 'low': 'minor', 'info': 'info'}
...
TYPE_ALIAS = {
    'invalid_fragment': 'imprecision', 'nonstandard_naming': 'ambiguous',
    'fragment': 'imprecision', 'knowledge_error': 'factual_error',
    'potential_confusion': 'confusion', 'conflict': 'logic_error',
}
...
KB_FACT_CHECKS = [
    (r'java.*解释型|java.*解释执行.*语言|java.*纯解释', 'Java 是"编译为字节码 + JVM 解释执行"的混合型语言', '软件知识库 - 编程语言篇', 'critical'),
    (r'python.*编译型|python.*编译型语言', 'Python 是解释型语言（.pyc 只是字节码缓存，本质仍解释执行）', '软件知识库 - 编程语言篇', 'critical'),
    (r'(javascript|java).*脚本.*(编译|编译型)', 'JavaScript 是解释型/即时编译（JIT）语言，非传统编译型', '软件知识库 - 编程语言篇', 'major'),
    (r'http(?!s).*(?<!不)(?<!无)(?<!未)加密|http(?!s).*自带加密', 'HTTP 本身不加密，HTTPS 通过 TLS 层实现加密', '软件知识库 - 网络全链路篇', 'critical'),
    (r'js.*单线程.*不能并发|node.*单线程.*不能并发|javascript.*不能并发', 'JavaScript 是单线程事件循环，但异步 I/O + Worker Threads 可支持并发', '软件知识库 - 编程语言篇', 'major'),
    (r'sql.*不支持.*查询|sql.*不能.*复杂', 'SQL 支持复杂查询（JOIN、子查询、窗口函数、CTE 等）', '软件知识库 - 数据库篇', 'major'),
    (r'jvm.*内存.*只有.*堆|jvm.*内存.*只有.*栈', 'JVM 内存区域包括：堆、栈、方法区、程序计数器、本地方法栈', '软件知识库 - 编程语言篇', 'critical'),
    (r'栈.*存储.*堆|堆.*存储.*栈|gc.*回收.*栈|栈.*垃圾回收', '栈(Stack)与堆(Heap)是独立内存区域；GC 只作用于堆，栈由调用帧自动管理', '软件知识库 - 内存模型篇', 'critical'),
    (r'tcp.*无连接|udp.*面向连接', 'TCP 是面向连接的协议，UDP 是无连接协议', '软件知识库 - 网络传输篇', 'critical'),
    (r'索引.*减慢.*查询|索引.*降低.*查询.*速度', '索引通过 B+树等结构加速查询，但会增加写入开销', '软件知识库 - 数据库篇', 'critical'),
]
...
def _issue(sentence, details):
    ...
    return {
        'type': type_code, 'severity': normalize_severity(details.get('severity') or meta['severity']),
        'text': sentence, 'entity': details.get('entity'),
        'description': details.get('description') or meta.get('label'),
        'correction': details.get('correction') or '', 'evidence': details.get('evidence') or '',
        'source': details.get('source') or 'realtime',
        'matchStart': max(0, rng[0]), 'matchEnd': max(0, rng[1]),
    }
...
def _check_sentence(sentence, db=None):
    issues = []
    ...
    for pattern, correction, evidence, severity in KB_FACT_CHECKS:
        ...
        if m:
            ...
            if not any(c in lower_text[max(0, hit_pos - 6):hit_pos] for c in '不无未非'):
                issues.append(_issue(text, {'type': 'factual_error', 'severity': severity,
                    'description': '表述与知识库相悖', 'correction': correction, 'evidence': evidence,
                    'source': 'kb_fact_check', 'range': (m.start(), m.end() or m.start() + 1)}))
    for info in OUTDATED_TERMS:
        r = _locate_term(text, info['term'])
        if r:
            issues.append(_issue(text, {'type': 'outdated', 'entity': info['term'],
                'description': '"%s" 已被 "%s" 取代（%s）' % (info['term'], info['replacement'], info['since']),
                'correction': '建议使用 "%s" 替代 "%s"' % (info['replacement'], info['term']),
                'evidence': info['reason'], 'source': 'outdated_terms', 'range': r}))
    for a, b, reason in CONFUSION_PAIRS:
        r = _locate_pair(text, a, b)
        if r and RELATION_WORDS.search(text):
            issues.append(_issue(text, {'type': 'confusion', 'entity': '%s/%s' % (a, b),
                'description': '"%s" 与 "%s" %s' % (a, b, reason),
                'correction': '请区分 "%s" 与 "%s"，避免混为一谈' % (a, b),
                'evidence': '软件知识库 - 概念辨析', 'source': 'confusion_pairs', 'range': r}))
    for term, ctx in AMBIGUOUS_TERMS:
        r = _locate_term(text, term)
        if r and not ctx.search(lower_text):
            issues.append(_issue(text, {'type': 'ambiguous', 'entity': term,
                'description': '"%s" 存在歧义（技术术语 vs 日常用语），上下文未明确指向' % term,
                'correction': '建议补充领域限定词，如 "%s 语言/框架/命令"' % term,
                'evidence': '软件知识库 - 术语表', 'source': 'ambiguous_terms', 'range': r}))
    for pattern, msg, suggest in IMPRECISION_PATTERNS:
        m = re.search(pattern, lower_text)
        if m:
            issues.append(_issue(text, {'type': 'imprecision', 'description': msg, 'correction': suggest,
                'evidence': '表述规范 - 知识完整性', 'source': 'imprecision',
    ...
    if total_units < 8 and not _ASCII_ONLY_RE.match(text):
        issues.append(_issue(text, {'type': 'imprecision', 'entity': None,
            'description': '碎片化断言（有效信息约 %d 个字符/词），缺少上下文或来源' % total_units,
            'correction': '补充更多上下文或出处后再保存', 'evidence': '内容完整性检测', 'source': 'fragment_check'}))
    if CERTAINTY_WORDS.search(text) and UNCERTAINTY_WORDS.search(text):
        issues.append(_issue(text, {'type': 'logic_error',
            'description': '句内同时出现绝对性断言与不确定性限定，存在逻辑矛盾',
            'correction': '删除相互矛盾的限定词，只保留确定口径',
            'evidence': '逻辑一致性检查', 'source': 'internal_consistency'}))
    tokens = _entity_tokens(text)
    if len(tokens) >= 2 and RELATION_DENY.search(text):
        issues.append(_issue(text, {'type': 'relation_error', 'entity': '/'.join(tokens[:2]),
            'description': '断言与知识库实体关系存疑：本体中相关实体被表述为无关',
            'correction': '请核实实体间真实关系后再断言',
            'evidence': '知识库桥接检测', 'source': 'relation_hint'}))
    if db is not None:
        issues.extend(_check_kb_opposites(lower_text, tokens, db))
    return issues


def _check_kb_opposites(lower_text, tokens, db):
    from app.models.models import KnowledgeBase
    issues = []
    if not tokens:
        return issues
    try:
        rows = db.query(KnowledgeBase).filter(KnowledgeBase.entity.in_(tokens)).all()
    except Exception:
        return issues
    for kb in rows:
        ...
        for opp in opposites:
            ...
            if opp_l in lower_text:
                ...
                if has_assert:
                    ...
                    issues.append(_issue(lower_text, {'type': 'factual_error', 'severity': 'critical',
                        'entity': kb.entity, 'description': '与知识库中"%s"的定义冲突（涉及对立术语"%s"）' % (kb.entity, opp),
                        'correction': kb.definition or '请核实"%s"与"%s"的真实关系' % (kb.entity, opp),
                        'evidence': kb.source or '知识库', 'source': 'kb_opposite', 'range': r or (0, len(kb.entity))}))
                else:
                    r = _locate_term(lower_text, kb.entity)
                    issues.append(_issue(lower_text, {'type': 'confusion', 'severity': 'major',
                        'entity': kb.entity, 'description': '句中同时出现"%s"及其对立概念"%s"，请注意区分' % (kb.entity, opp),
                        'correction': '核实该概念与"%s"的关系后重新表述' % opp,
                        'evidence': kb.source or '知识库', 'source': 'kb_opposite', 'range': r or (0, len(kb.entity))}))
    return issues


def split_sentence_ranges(content):
    out = []
    text = content or ''
    if not text.strip():
        return out
    ...
    for m in re_chunk.finditer(text):
        ...
        if s0 >= s1:
            continue
        out.append({'sentence': text[s0:s1], 'start': s0, 'end': s1, 'line': text[:s0].count('\n')})
    return out


def validate_content(content, db=None):
    issues = []
...
def summarize(issues, sentence_count=None):
    critical = sum(1 for i in issues if i['severity'] == 'critical')
    major = sum(1 for i in issues if i['severity'] == 'major')
    minor = sum(1 for i in issues if i['severity'] == 'minor')
    info = sum(1 for i in issues if i['severity'] == 'info')
    total = max(int(sentence_count or len(issues)), 1)
    deduction = sum({'critical': 1.0, 'major': 0.6, 'minor': 0.25, 'info': 0.05}.get(i['severity'], 0) for i in issues)
    accuracy = max(0, min(100, round((1 - deduction / total) * 100)))
    return {
        'total': total, 'passed': max(0, total - critical - major - minor - info),
        'errors': critical + major, 'warnings': minor + info,
        'critical': critical, 'major': major, 'minor': minor, 'info': info, 'accuracy': accuracy,
    }
