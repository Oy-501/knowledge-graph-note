...
def save_note(request: NoteSaveRequest, db: Session = Depends(get_db)):
    ...
    if request.id:
        ...
        note = Note(
            ...
            tags=request.tags or [], linked_files=request.linked_files or [],
            linked_nodes=request.linked_nodes or [], created_at=now, updated_at=now
        )
        if request.archived is not None:
            note.archived = request.archived
        _apply_validation(note, request)
    ...
    return {
        ...
        "note": {
            ...
            "tags": note.tags, "linked_files": note.linked_files, "linked_nodes": note.linked_nodes,
            "archived": note.archived, "accuracy_score": note.accuracy_score,
            "verified": note.verified, "validation_status": note.validation_status,
        ...
        "notes": [
            {
                ...
                "tags": n.tags or [], "linked_files": n.linked_files or [],
                "linked_nodes": n.linked_nodes or [], "archived": n.archived,
                "accuracy_score": n.accuracy_score, "verified": n.verified,
                "validation_status": n.validation_status, "validation_report": n.validation_report,
...
def get_note(note_id: int, user_id: int = 1, db: Session = Depends(get_db)):
    ...
    return {
        ...
        "note": {
            "id": note.id, "title": note.title, "content": note.content,
            "tags": note.tags or [], "linked_files": note.linked_files or [],
            "linked_nodes": note.linked_nodes or [], "archived": note.archived,
            "accuracy_score": note.accuracy_score, "verified": note.verified,
            "validation_status": note.validation_status, "validation_report": note.validation_report,
            "created_at": note.created_at.isoformat() if note.created_at else None,
            "updated_at": note.updated_at.isoformat() if note.updated_at else None
        }
    }
