...
def build_graph(options: GraphOptions = None, db: Session = Depends(get_db)):
    ...
    if node_ids:
        links = db.query(Link).filter(Link.source_id.in_(node_ids), Link.target_id.in_(node_ids)).all()
    else:
        links = []
    return {
        "nodes": [{
            "id": n.id, "fileId": n.file_id, "title": n.title or n.entity,
            "entity": n.entity, "description": n.description or "",
            "keywords": n.keywords or [], "entities": n.entities or [], "type": n.type,
            "level": n.level, "levelLabel": n.level_label, "domain": n.domain,
            "groupId": n.group_id, "groupName": n.group_name, "status": n.status,
            "validated": n.validated, "validateStatus": n.validate_status,
            "confidence": n.confidence, "visible": n.visible,
            "isolateBlackList": n.isolate_blacklist or [], "uploadTime": n.upload_time,
            "validate": {"status": n.validate_status or "pending", "issues": [], "aiFix": None,
                         "manualEdit": None, "confirmedAt": None}
        } for n in nodes],
        "links": [{
            "id": l.id, "source": l.source_id, "target": l.target_id,
            "relation_type": l.relation_type, "relation_label": l.relation_label,
            "relation_color": l.relation_color, "relation_lineStyle": l.relation_line_style,
            "score": l.score, "final_weight": l.final_weight, "is_render": l.is_render,
            "evidence": l.evidence, "source_text": l.source_text, "source_file": l.source_file,
            "auto_generated": l.auto_generated, "user_confirmed": l.user_confirmed,
            "semantic_bridge": l.semantic_bridge, "timeBridge": l.time_bridge
        } for l in links]
    }
...
def rebuild_graph(data: dict, db: Session = Depends(get_db)):
    ...
    for i, f in enumerate(files):
        ...
        links = infer_links_batch(new_dicts, user_id, weights, threshold, db, existing_nodes=existing_dicts)
