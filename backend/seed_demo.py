...
用于验证前端功能。可重复运行（幂等：先清空旧 nodes/links/files）。
"""
import sqlite3
import json
import time
...
REL_COLORS = {"prerequisite": "#D4A574", "related": "#8A9AA8", "contains": "#7CB8A0", "extension": "#8A9AA8", "theory": "#4F6F8F"}
...
def main():
    ...
    for fid, name in file_names.items():
        cur.execute("INSERT INTO files (user_id, name, content, status, node_count) VALUES (1, ?, '', 'done', 0)", (name,))
    ...
    for name, fid, entity, title, ntype, level, domain, keywords, desc, days, vstatus in NODE_DEFS:
        cur.execute(
            ...
            (file_ids[fid], entity, title, ntype, desc, json.dumps(keywords, ensure_ascii=False),
             json.dumps(keywords, ensure_ascii=False), level, LEVEL_LABELS[level], domain,
             1 if vstatus == "passed" else 0, vstatus, days_ago(days)),
        )
    ...
    for a, b, rtype, rlabel, score, evidence in LINK_DEFS:
        ...
        cur.execute(
            ...
            (node_ids[a], node_ids[b], rtype, rlabel, REL_COLORS.get(rtype, "#8A9AA8"), score, score, evidence),
        )
        link_count += 1
    conn.commit()
